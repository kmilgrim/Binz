...
class users(db.Model): #class of users
    __tablename__='users'
...
@app.route("/", methods=["POST", "GET"]) #login page info
...
def login():
    if request.method == "POST": # if user submits
        if request.form['submit_button'] == 'Sign In':
            ...
            if found_user: #get the current user from database
        ...
        if "user" in session:
            ...
            return redirect(url_for("home"))
        return render_template("login.html")#if none of the above, return login
...
def home():
    if "user" in session:
        ...
        name = []
        points = []
        for i in range(0,3): #sort users, get first 3 names
            ...
            try:
                ...
                name.append("N/A")
                points.append(0)
                # go home with user data and leaderboard data
...
@app.route("/logout") #pop user and go to login
def logout():
    if "user" in session:
        user = session["user"]
...
@app.route("/deleet")#pop user and go to login and delete from database
def deleet():
    if "user" in session:
        user = session["user"]
        if user == "Guest":
...
def recycle():
    if request.method == "POST": #get current user if exists and add 10
        if "user" in session:
            user = session["user"]
